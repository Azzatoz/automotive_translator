...
| `roadmap.md` | Навигация по проекту                       |
...
| `main_window.py`                             | Окно: сайдбар модулей, вкладки, оркестрация панелей        |
| `config.py`                                  | Пути к скриптам, словарям, пресеты папок, индексы вкладок  |
| `module_sidebar.py`                          | Список модулей: фильтр, сортировка, PKM-меню               |
| `translate_panel.py`                         | Вкладка **Обзор**: fill APK / словарь, область модулей     |
| `layout_panel.py`                            | Вкладка **Действия**: хардкод в layout → strings           |
| `dictionary_panel.py`                        | Вкладка **Словарь**: заглушки, pending, отчёт Google       |
| `conflicts_panel.py`                         | Вкладка **Конфликты**: выбор варианта, запись в APK        |
| `placeholders_dialog.py`                     | Редактор заглушек модуля: APK, словарь, undo, похожие      |
| `dictionary_data.py`                         | Загрузка словарей, pending, Google-отчёта; открытие файлов |
| `module_align.py` / `module_align_dialog.py` | Сверка APK ↔ словарь                                       |
| `scanner.py`                                 | Сканирование `*_src`, статистика, бейджи                   |
| `process.py`                                 | Очередь `QProcess`, лог stdout/stderr                      |
| `workers.py`                                 | Фоновое сканирование (`QThread`)                           |
| `backup.py`, `confirm.py`, `faq.py`          | Бэкап values-ru, опасные действия, справка                 |
| `theme.py`, `responsive.py`, `widgets.py`    | Тема, адаптивная вёрстка, виджеты                          |


**Вкладки:** Обзор · Действия · Конфликты · Словарь · Лог.
...
| Файл                            | Назначение                                        |
| ------------------------------- | ------------------------------------------------- |
| `translation_ecosystem_map.svg` | Схема связей скриптов, словарей и GUI (`gui_pkg`) |
...
`python3 gui.py` → папка `Translated` → модуль слева:


| Вкладка       | Содержимое                                                                             |
| ------------- | -------------------------------------------------------------------------------------- |
| **Обзор**     | Прогресс проекта/модуля, `translate_panel`, быстрые кнопки (заглушки, collect, layout) |
| **Действия**  | `layout_panel`, сортировка/аудит/даты словаря                                          |
| **Конфликты** | Выбор варианта перевода, запись в словарь и APK                                        |
| **Словарь**   | Заглушки в JSON, pending, строки из отчёта Google fill                                 |
| **Лог**       | Вывод скриптов                                                                         |


PKM по модулю: заглушки, fill одного модуля, APK↔словарь, проводник.
